...
import com.example.controladorgastos.adaptador.incomeAdapter;
...
AddGastos extends Fragment {
    ...
    private void fillExpenseModel() {
        gastoList = databaseHandlerExpense.getAllIncome();

        int total = 0;
        for (Gasto model : gastoList) {
            total += Integer.parseInt(model.getAmount());
        }
        totalExpense = String.valueOf(total);
        tv_expense.setText("₹" + totalExpense);

        expenseAdapter = new expenseAdapter(getContext(), gastoList);
    ...
    private void fillIncomeModel() {
        usuarioList = databaseHandler.getAllIncome();
        ...
        incomeAdapter = new incomeAdapter(getContext(), usuarioList);
